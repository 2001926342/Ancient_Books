...
if __name__ == "__main__":
    DATA_ROOT = Path("/group_share/Ancient_Books/dataset")  # 这里指定数据集的根目录路径
    OUTPUT_PATH = "data.jsonl"  # 这里指定输出的文件路径

    if not DATA_ROOT.exists():
        raise FileNotFoundError(f"Can't not found data root {DATA_ROOT}")

    SYSTE_STR = "你是一位古籍解读专家。你总能解答用户关于中文的相关知识。"
    
    # 文言文白话文数据集
    DATA_ROOT_ERYA = DATA_ROOT.joinpath(r"/group_share/Ancient_Books/dataset/Erya-dataset")
    get_erya_data(
        DATA_ROOT_ERYA,
